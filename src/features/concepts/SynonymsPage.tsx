...
import { getHanjaByCharacter, synonymItems } from "../../data";
...
import type { SynonymCharacter } from "../../shared/types/concepts";
...
export function SynonymsPage() {
  ...
  const sessionEntries = useMemo(
    () => synonymItems.map((item) => ({ ...item, id: String(item.id) })),
    [],
  );
  ...
  useEffect(() => {
    setConceptProgress(CONCEPT_IDS.synonyms, index);
    ...
    <Screen noPadding className="pb-24">
      <div className="px-4 pt-4">
        <ConceptSessionHeader
          ...
          listPath={buildConceptListPath(CONCEPT_IDS.synonyms, index)}
        />
