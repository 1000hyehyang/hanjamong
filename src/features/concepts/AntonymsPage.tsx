...
import { antonymItems, getHanjaByCharacter } from "../../data";
...
export function AntonymsPage() {
  ...
  const sessionEntries = useMemo(
    () => antonymItems.map((item) => ({ ...item, id: String(item.id) })),
    [],
  );
  ...
  useEffect(() => {
    setConceptProgress(CONCEPT_IDS.antonyms, index);
    ...
    <Screen noPadding className="pb-24">
      <div className="px-4 pt-4">
        <ConceptSessionHeader
          ...
          listPath={buildConceptListPath(CONCEPT_IDS.antonyms, index)}
        />
